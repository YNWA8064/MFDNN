...
# Get an MNIST image
testset = torchvision.datasets.MNIST(root='./../DATA/mnist_data/', train=False, download=False, transform=torchvision.transforms.ToTensor())
...
for i in range(300):
    recon.retain_grad()
    p_x = -model(recon.view(-1, 784))
    p_x.backward()

    recon = recon - lr * recon.grad

    recon.data[mask] = image[mask]
    recon.data[mask.logical_not()] = recon.data[mask.logical_not()].clamp(min=0, max=1)
